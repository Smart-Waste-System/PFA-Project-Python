...
// Intercepteur global pour gérer l'expiration du token
...
export default function App() {
    ...
    // Décodage du token pour extraire le rôle et les infos utilisateur
    useEffect(() => {
        if (token) {
            try {
                    ...
                    >
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl px-4 py-3 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none transition-all"
                                placeholder="Email professionnel"
                                autoComplete="off"
                                name="smart-waste-email"
                                required
                            />
                        </div>
                        ...
                        <button
                            type="submit"
                            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-emerald-900/40 mt-4"
                        >
                            Se connecter
                        </button>
                    </form>
    ...
    // 2. SI CHAUFFEUR : interface mobile chauffeur
    if (userRole === 'DRIVER') {
        return (
            <DriverMobileView
                user={{
    ...
    // 3. SI ADMIN OU SUPER_ADMIN : dashboard
    return (
        <MapDashboard
            user={{
                email: userProfile.email || email,
